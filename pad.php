...
<head>
    <meta charset="utf-8">
    <title>Association SPA - Pad d'activité</title>
            ...
            <div class="activiteALL">
                <?php
                    if (isset($_GET['code'])){
                        ...
                        echo("vous avez oublié le code comme paramètre");
                        echo("<br>");
                    }
                    $t_pad_activite_pad ='SELECT * FROM t_pad_activite_pad
                    WHERE pad_code ="'.$pad_code.'";';
                    $result2 = $mysqli->query($t_pad_activite_pad);

                    if ($result2 == false) //Erreur lors de l’exécution de la requête
                    { // La requête a echoué
                    echo "Error: La requête a echoué 2 \n";
                    echo "Errno: " . $mysqli->errno . "\n";
                    echo "Error: " . $mysqli->error . "\n";
                    exit();
                    }
                    $activite = $result2->fetch_assoc();
                ?>
                <h1>Activitées du pad <h1>
                    <h6 style="color: #ED6436; font-size: 32pt;"><?php echo($activite['pad_intitule']);?></h6>
                <?php
                    
                    $t_ressource_res='SELECT * FROM t_ressource_res JOIN t_atelier_ate USING (ate_id) JOIN t_pad_activite_pad USING (pad_id)
                    WHERE pad_code ='.'"'.$pad_code.'"'.';';
                    $result3 = $mysqli->query($t_ressource_res);
                    if ($result3 == false) //Erreur lors de l’exécution de la requête
                    { // La requête a echoué
                        echo "Error: La requête a echoué 3 \n";
                        echo "Errno: " . $mysqli->errno . "\n";
                        echo "Error: " . $mysqli->error . "\n";
                    exit();
                    }
                    $t_associe_asc ="SELECT * FROM t_associe_asc WHERE pad_id=".$activite['pad_id'].";";
                    $result4 = $mysqli->query($t_associe_asc);
                    ...
                    echo('<div id="activite">');
                    echo("<table class=tabActivite><tr><th><h4>Intitulé de l'atelier</h4></th><th><h4>Intitulé de la ressource</h4></th><th><h4>Lien hypertexte vers la ressource</h4></tr>");
                    
                    while($res = $result3->fetch_assoc()){
                        //echo("<h4>Pad de l'activité"." '".$pad['pad_intitule']."'"." -- ".$res['res_type_ressource']." -- <a href=".$res['res_chemin_acces'].">".$res['res_titre']."</a></h4>");
                        // echo ('<br>');
                        
                        echo("<tr>");
                        echo ('<td>'.$res['ate_intitule'].'</td>');
                        echo('<td>'.$res['res_titre'].'</td>');
                        echo('<td><a href="'.$res['res_chemin_acces'].'"target="_blank">'.$res['res_titre']."</a></td>");
                        echo("</tr>");
                    }
                    
                    echo("</table>");
                    echo("<br>");
                    echo("<h5>Animat-eur(s)/trice(s) :</h5>");
                    echo("<br>");
                    while($associe = $result4->fetch_assoc()){
                        echo("<h6>- ".$associe['com_pseudo']."</h6>");
                    }
                    echo('</div>');
                    $mysqli -> close();
                ?>
